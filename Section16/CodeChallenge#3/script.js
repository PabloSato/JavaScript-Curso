...
const loadNPause = async function () {
  try {
    ...
    await wait(2); //Esta función no devuelve nada, asique no neesitas almacenarla en ninguna variable
...
const loadAll = async function (imgArr) {
  try {
    //Como createImage es asincrona, para poder utilizarla dentro
    //del map necesitas meter el async/await dentro también
    const imgs = imgArr.map(async img => await createImage(img));
    // console.log(imgs); // No pinta las imagenes, pinta las promesas (async siempre devuelve promesas no el valor)

    const imgsEl = await Promise.all(imgs);
    console.log(imgsEl); // Esto SI que devuelve las imagenes
